...
//Code for updating player names
$("#player-1-name").text(sessionStorage.getItem("one-vs-three-player-1-name"));
$("#player-2-name").text(sessionStorage.getItem("one-vs-three-player-2-name"));
$("#player-3-name").text(sessionStorage.getItem("one-vs-three-player-3-name"));
$("#player-4-name").text(sessionStorage.getItem("one-vs-three-player-4-name"));
      ...
      if (count == randomDice) {
        ...
        if (
          ...
          if (
            x == sessionStorage.getItem("one-vs-three-computer-player-1") ||
            x == sessionStorage.getItem("one-vs-three-computer-player-2") ||
            (x == sessionStorage.getItem("one-vs-three-computer-player-3") &&
...
function findWinner() {
  if (identifyPlayer == ".r") {
    gameParticipants = [".b57", ".y57", ".g57"]; // Game participants excludes current player or player with dice
  } else if (identifyPlayer == ".b") {
    gameParticipants = [".r57", ".y57", ".g57"];
  } else if (identifyPlayer == ".y") {
    gameParticipants = [".r57", ".b57", ".g57"];
  } else if (identifyPlayer == ".g") {
    gameParticipants = [".r57", ".b57", ".y57"];
  }
  ...
  //Code for declaring third winner
  if (
    ($(`td${gameParticipants[0]}`).find("img").length == 4 &&
      $(`td${gameParticipants[1]}`).find("img").length == 4 &&
      $(`td${gameParticipants[2]}`).find("img").length != 4) ||
    ...
    if ($(`td${identifyPlayer}57`).find("img").length == 4) {
      ...
      if ($(`td.r57`).find("img").length != 4) {
        fourthWinner = $("#player-1-name").text();
        xOfFourth = 1;
      } else if ($(`td.b57`).find("img").length != 4) {
        fourthWinner = $("#player-2-name").text();
        xOfFourth = 2;
      } else if ($(`td.y57`).find("img").length != 4) {
        fourthWinner = $("#player-3-name").text();
        xOfFourth = 3;
      } else if ($(`td.g57`).find("img").length != 4) {
        fourthWinner = $("#player-4-name").text();
        xOfFourth = 4;
      }

      //Code for Game complete message
      setTimeout(function () {
...
function fiveMergeCode() {
  ...
  for (let i = 1; i < 52; i++) {
    if (result == undefined) {
      ...
      if (result == true) {
        ...
        } else if (resultLocation4 == undefined) {
          resultLocation4 = i;
          resultLocation4ExchH1 = $(`td.r${resultLocation4} > span > span:nth-child(2) > img:nth-child(2)`)[0];
          resultLocation4ExchH2 = $(`td.r${resultLocation4} > span > span:nth-child(2) > img:nth-child(3)`)[0];
        }
        result = undefined;
      } else {
        result = undefined;
  ...
  if (
    ...
    } else if (resultLocation4ExchH2.classList[1] == identifyColor) {
      mergeExchClass = ".r" + resultLocation4;
      mergeExchHorse1 = resultLocation4ExchH1;
      mergeExchHorse2 = resultLocation4ExchH2;
    }
  }
}
...
function computerHorse() {
  if (lastClick >= Date.now() - delay) {
    ...
    if (
      x == sessionStorage.getItem("one-vs-three-computer-player-1") ||
      x == sessionStorage.getItem("one-vs-three-computer-player-2") ||
      (x == sessionStorage.getItem("one-vs-three-computer-player-2") && sessionStorage.getItem("computer") != null)
    ) {
      setTimeout(() => {
