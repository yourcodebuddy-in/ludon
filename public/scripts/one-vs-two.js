// Restrict direct access to game board
// const referrer = document.referrer;
// if (referrer !== `${window.location.origin}/pass-and-play` && referrer !== `${window.location.origin}/computer`) {
...
$(`#player-${sessionStorage.getItem("one-vs-two-player-1")}-name`).text(
  sessionStorage.getItem("one-vs-two-player-1-name")
);
$(`#player-${sessionStorage.getItem("one-vs-two-player-2")}-name`).text(
  sessionStorage.getItem("one-vs-two-player-2-name")
);
$(`#player-${sessionStorage.getItem("one-vs-two-player-3")}-name`).text(
  sessionStorage.getItem("one-vs-two-player-3-name")
);

//Code for deleting content of extra players

//Code for assigning dice to first player of 1v3 mode
x = sessionStorage.getItem("one-vs-two-player-1");

// Code for removing extra players dice and image
$(`#player-${sessionStorage.getItem("one-vs-two-not-selected-player")}-area`).css("opacity", 0);

//Code for removing extra player horses & labels
$(`#player-${sessionStorage.getItem("one-vs-two-not-selected-player")} > table img`).remove();
$(`#player-${sessionStorage.getItem("one-vs-two-not-selected-player")} > p`).css("opacity", 0);

//Code for changing default player labels
$(`#player-${sessionStorage.getItem("one-vs-two-player-1")} > p`).text("Player 1");
$(`#player-${sessionStorage.getItem("one-vs-two-player-2")} > p`).text("Player 2");
$(`#player-${sessionStorage.getItem("one-vs-two-player-3")} > p`).text("Player 3");
...
document.addEventListener("game-countdown-complete", function () {
  ...
  if (
    ...
    (x == sessionStorage.getItem("one-vs-two-computer-player-2") && sessionStorage.getItem("computer") != null)
  ) {
    setTimeout(() => {
...
function transferDiceCode() {
  if (x <= 4) {
    ...
    if (x == 5 && sessionStorage.getItem("one-vs-two-player-3") == "1") {
      x = 1;
    } else if (x == 5 && sessionStorage.getItem("one-vs-two-player-3") == "2") {
      x = 2;
    } else if (x == 5) {
      x = sessionStorage.getItem("one-vs-two-player-1");
    }
    ...
    if (x == 1 && sessionStorage.getItem("one-vs-two-not-selected-player") == 1) {
      x = 2;
    } else if (x == 2 && sessionStorage.getItem("one-vs-two-not-selected-player") == 2) {
      x = 3;
    } else if (x == 3 && sessionStorage.getItem("one-vs-two-not-selected-player") == 3) {
      x = 4;
    } else if (x == 4 && sessionStorage.getItem("one-vs-two-not-selected-player") == 4) {
      x = 1;
    }
  ...
  if (resultLocation1 != undefined || resultLocation2 != undefined) {
    if (mergeExchHorse1.classList[1] == identifyColor && mergeExchHorse2.classList[1] != identifyColor) {
      ...
      if (mergeExchHorse3.classList[1] != identifyColor) {
        ...
        $("td" + mergeExchClass + " > span:first-child > span:nth-of-type(2) > img:nth-child(3)").css(
          "display",
          "none"
        );
      }
...
$(`.player-dice`).click(function () {
  if (d == 0) {
    ...
    $(this).attr("src", "gifs/dice-roll.gif"); //Assigns a dice roll gif on dice click
    ...
    setTimeout(function () {
      ...
      switch (randomDice) {
        ...
        case 2:
          $("#" + playerDice).attr("src", "dice/dice-2.png"); //Assigns a dice 2 image
          $("#player-" + x + "-dice-arrow").attr("src", "");
          accurateMoveHorse();
          highlightHorses();
        ...
        case 3:
          $("#" + playerDice).attr("src", "dice/dice-3.png"); //Assigns a dice 3 image
          $("#player-" + x + "-dice-arrow").attr("src", "");
          accurateMoveHorse();
          highlightHorses();
          setTimeout(transferDice, 300);
        ...
        case 4:
          $("#" + playerDice).attr("src", "dice/dice-4.png"); //Assigns a dice 4 image
          $("#player-" + x + "-dice-arrow").attr("src", "");
          accurateMoveHorse();
          highlightHorses();
          setTimeout(transferDice, 300);
        ...
        case 5:
          $("#" + playerDice).attr("src", "dice/dice-5.png"); //Assigns a dice 5 image
          $("#player-" + x + "-dice-arrow").attr("src", "");
          accurateMoveHorse();
          highlightHorses();
          setTimeout(transferDice, 300);
        ...
        case 6:
          $("#" + playerDice).attr("src", "dice/dice-6.png"); //Assigns a dice 6 image
          $("#player-" + x + "-dice-arrow").attr("src", "");
...
function moveHorse(event) {
  ...
  if ($(`.path`).find(`img.${identifyColor}`).length >= 1) {
    ...
    } else if (selectedPathCell.length == 11) {
...
function accurateMoveHorse() {
  ...
  autoHorsesLength = autoHorses.filter(function (val) {
    return val == true;
  }).length;
...
function moveDice() {
  $("#player-" + x + " img").click(function (event) {
    if (randomDice == 6 && z < 2) {
      ...
      if (
        "." + userSelectedHorse == identifyPlayer + "h1" ||
...
function unMergeHorses() {
  if ($(mergeHorseClassLast).find("img").length == 1) {
    ...
    if ($("td" + mergeHorseClassLast + " > span > span:first-child").html() == "") {
      ...
      $("td" + mergeHorseClassLast + " > span > span:first-child").html() != "" &&
    ...
    $(mergeHorseClassLast).find("img").length == 4 &&
...
function findWinner() {
  ...
  gameParticipants.push(sessionStorage.getItem("game-participants-1"));
  gameParticipants.push(sessionStorage.getItem("game-participants-2"));
  gameParticipants.push(sessionStorage.getItem("game-participants-3"));
...
function fiveMergeCode() {
  ...
  if (
    (resultLocation1ExchH1 != undefined && resultLocation1ExchH2 != undefined) ||
...
function computerHorse() {
  if (lastClick >= Date.now() - delay) {
    ...
    if (
      ...
      setTimeout(() => {
        if ($(`#player-${x} > table`).find("img").length != 0 && randomDice == 6) {
          ...
          if (computerHorsesOnPath.length == 2) {
            ...
            for (let i = 1; i < 3; i++) {
              if (window[`newPosCH${i}`] < 57) {
                if (
                  $(identifyPlayer + window[`newPosCH${i}`]).find("img").length == 1 &&
                  ...
                  for (let j = 1; j < 3; j++) {
                    if (
                      ...
                      for (let k = 1; k < 3; k++) {
                        if (
                          window[`lastPos${window[`computerHorse${k}`]}`] != 1 &&
